  ...
  methods: {
    ...
    execute: function () {
      ...
      switch (this.params.mode) {
        case "set": {
          this.$http.validate_set(this.params.targetID, this.params.target, this.params.runs, 100, this.params.distance).then(response => {
        ...
        case "id-set": {
          this.$http.validate_id_set(this.params.targetID, this.params.target, this.params.referenceID, this.params.reference, this.params.runs, 100, this.params.enriched, this.params.distance).then(response => {
            this.saveResult(response)
          }).catch(() => {
            this.error = true
          })
        ...
        case "set-set": {
          this.$http.validate_set_set(this.params.targetID, this.params.target, this.params.referenceID, this.params.reference, this.params.runs, 100, this.params.enriched, this.params.distance).then(response => {
            this.saveResult(response)
          }).catch(() => {
            this.error = true
          })
