    ...
    <v-card :style="{'background-color': '#d4e6f5', width: '100%', padding: mobile ? '16px' :'32px', display: 'flex'}">
      ...
      <div style="width: 60%; justify-content: flex-end; margin-left: auto; margin-right: 0">
        <div style=" height: 100%;display:flex;">
          <p :style="{'font-size': mobile ? '1rem' : '1.3rem', color: '#1c388a', 'align-self': 'flex-end', 'margin-top': 'auto', 'margin-bottom': 0}">In silico validation of disease and gene sets, clusterings, or subnetworks.</p>
        </div>
      </div>
    ...
    <div
      ...
      <template v-if="step===0">
        <div style="display: flex; justify-content: center; width: 100%">
          <v-img src="/static/assets/digest_summary_illustration.avif" :max-width="mobile ? '90%' : '70%'"/>
<!--          <v-img :src="getIllustration()" :max-width="mobile ? '90%' : '70%'"></v-img>-->
        </div>
      </template>
    </div>
    <div v-if="step===0">
